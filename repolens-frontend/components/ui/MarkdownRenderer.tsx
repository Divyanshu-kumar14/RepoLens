  ...
  while (k < blocks.length) {
    ...
    if (block.type === "bullet") {
      ...
      rendered.push(
        <ul
          key={`bullet-${startK}`}
          className="list-none space-y-1.5 my-2 pl-1"
        >
          {items.map((item, idx) => (
            <li
              key={idx}
    ...
    if (block.type === "h1") {
      rendered.push(
        <h2
          key={`h1-${startK}`}
          className="text-base font-bold mt-4 mb-1.5 text-gray-900 dark:text-gray-100"
        >
    ...
    if (block.type === "h2") {
      rendered.push(
        <h3
          key={`h2-${startK}`}
          className="text-sm font-bold mt-3 mb-1 text-gray-800 dark:text-gray-200 border-b border-gray-200 dark:border-gray-700 pb-0.5"
        >
    ...
    if (block.type === "h3") {
      rendered.push(
        <h4
          key={`h3-${startK}`}
          className="text-sm font-semibold mt-2 mb-0.5 text-blue-700 dark:text-blue-400"
        >
    ...
    if (block.type === "paragraph") {
      ...
      // Using [\s\S]* instead of the /s flag for ES2017 compatibility.
      const labelMatch = pb.text.match(/^([A-Z][^:]{0,40}):\s*([\s\S]*)/);
